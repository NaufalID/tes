...
const listMessage = {
  ...
  footer: '© created by naufalmuflih',
...
conn.sendMessage(from, { caption: cap, image: { url: feta.data.qris }, buttons: buto, footer: '© created by naufalmuflih' })
}
console.log(feta)
}
break
...
if (!isGroup) return reply(mess.OnlyGrup)
                    ...
                    anu = await fetchJson(`https://api.xteam.xyz/game/tebakangka?q=${F}&APIKEY=apikeyaine`)
        ...
        case 'tebakbendera':  
        if (!isGroup) return reply(mess.OnlyGrup)
			              anu = await fetchJson(`https://api.xteam.xyz/game/tebakbendera?APIKEY=apikeyaine`)
