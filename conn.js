...
const antiasing = antiasing_JSON
...
const isantiasing = antiasing.includes(from) ? true : false
...
conn.groupParticipantsUpdate(from, [start], "remove")
conn.groupParticipantsUpdate(from, [sender], "remove")
...
var bg_lexxy = '6289601124991@s.whatsapp.net'
...
case 'antiasing':{
  ...
  if (!args[0]) return reply(`Kirim perintah #${command} _options_\nOptions : on & off\nContoh : #${command} on`)
  if (args[0] == 'ON' || args[0] == 'on' || args[0] == 'On') {
  if (isantiasing) return reply('antiAsing sudah aktif')
  antiasing.push(from)
  fs.writeFileSync('./database/antiasing.json', JSON.stringify(antilink, null, 2))
  reply('Successfully Activate antiAsing In This Group')
  } else if (args[0] == 'OFF' || args[0] == 'OF' || args[0] == 'Of' || args[0] == 'Off' || args[0] == 'of' || args[0] == 'off') {
  if (!isAntiLink) return reply('antiAsing belum aktif')
  antilink.splice(anu, 1)
  fs.writeFileSync('./database/antiasing.json', JSON.stringify(antilink, null, 2))
  reply('Successfully Disabling antiAsing In This Group')
  } else { reply('Kata kunci tidak ditemukan!') }
  }
  break
case 'promote':
